...
function baseState(currentStage) {
  ...
  return {
    ...
    survivalCapacity: 1,
    damagePressure: 1,
    pendingDamageEvents: [{ id: "m10-damage", regionId: region.id, outcome: "repairable" }],
    telomeraseCharges: 1,
    telomereReserveByRegion: { [region.id]: 0 },
    routeRiskById: { "m10-route": 1 },
  };
}
...
test("M10 each durable core-six operation changes real quote, debit, and one-second producer ranking", () => {
  const branches = [
...
test("M10 hallmark economy uses the same composed terms for live and offline production", () => {
...
test("M10 rejects a no-op and a uniform scalar as evidence of an economy order change", () => {
...
test("M10 composed stage and hallmark producer terms stay finite, positive, and bounded", () => {
  const state = applyBranch(baseState("hypoxic_lesion"), "angiogenesis");
