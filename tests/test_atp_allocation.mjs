...
} from "../src/hallmarks/atp_allocation.ts";
...
test("extended-hallmark acceleration changes all eight real quotes nonuniformly, with a bounded rate payoff", () => {
...
test("extended-hallmark unfunded or zero acceleration cannot improve output or quotes", () => {
...
test("extended-hallmark acceleration pays once per second, after vessel maintenance, with no double debit", () => {
  const vessel = {
    id: regionId("extended-hallmark-vessel"),
    capacity: 4,
    viability: 1,
    phenotype: "proliferative",
    vesselLinkIds: [eventId("vessel:extended-hallmark-vessel")],
    routeIds: [],
  };
...
test("extended-hallmark allocation is segmentation invariant and matches live and offline replay", () => {
...
test("extended-hallmark ATP allocation rejects unknown sinks and a total above its canonical cap", () => {
  assert.throws(
    () =>
      economyTick(
...
test("vessel upkeep reserves ATP for every maintained link before acceleration", () => {
  const vessel = {
    id: regionId("atp-reservation-vessel"),
    capacity: 4,
    viability: 1,
    phenotype: "proliferative",
    vesselLinkIds: [eventId("vessel:atp-reservation")],
    routeIds: [],
  };
  const base = acceleratedState({
    hallmarkLevels: [
      { id: hallmarkId("angiogenesis"), level: 1 },
      { id: hallmarkId("metabolic_deregulation"), level: 1 },
    ],
  ...
  const underReserved = advanceLiveTick(
    {
      game: {
        ...base,
  ...
  const reserved = advanceLiveTick(
    {
      game: {
        ...base,
  ...
  assert.deepEqual(underReserved.game.regions[0]?.vesselLinkIds, []);
  assert.equal(reserved.game.regions[0]?.vesselLinkIds.length, 1);
});
