...
function region() {
  return {
    id: regionId("extended-hallmark-region"),
    capacity: 3,
    viability: 1,
    phenotype: "proliferative",
    vesselLinkIds: ["extended-hallmark-vessel"],
    routeIds: [],
  };
}
...
function roundTrip(state) {
  ...
  if (result.status !== "loaded")
    throw new Error("Expected current extended-hallmark save to load.");
...
test("extended-hallmark amended events parse exactly, dispatch once, and preserve save identity", () => {
  const metabolism = {
  ...
  const immune = {
    ...
    concealmentTokens: 1,
    immuneVisibilityByRegion: { "extended-hallmark-region": 1 },
  };
  const masked = recordEvent(immune, {
    type: "set-region-mask",
    regionId: regionId("extended-hallmark-region"),
    masked: true,
    atMs: 10,
  });
  assert.deepEqual(masked.maskedRegions, [regionId("extended-hallmark-region")]);
...
test("extended-hallmark parser and reducer reject hostile, stale, duplicate, and no-op records atomically", () => {
  const immune = {
    ...stateFor("immune_destruction_avoidance", stageId("angiogenic_primary")),
    concealmentTokens: 1,
    immuneVisibilityByRegion: { "extended-hallmark-region": 1 },
  ...
  atomicallyRejects(immune, {
    type: "set-region-mask",
    regionId: "extended-hallmark-region",
    masked: true,
    atMs: 10,
    extra: true,
  });
...
test("extended-hallmark conversion parser rejects hostile BigNum DTO ownership", () => {
  ...
  for (const amount of [
    ...
    atomicallyRejects(state, { type: "convert-substrate", amount, atMs: 10 });
  }
});

test("extended-hallmark save rejects impossible ATP, visibility, episode, and offer relations", () => {
  const initial = createInitialGameState();
  for (const state of [
    ...
    { ...initial, maskedRegions: [regionId("missing")] },
    {
      ...
      inflammationEpisodes: [
        { id: "episode", regionId: regionId("extended-hallmark-region"), deadlineMs: 0 },
      ],
    },
  ]) {
...
test("extended-hallmark save rejects forged or divergent chosen-mutation and liability graphs", () => {
