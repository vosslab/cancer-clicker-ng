...
async function earnAndBuyCyclin(page) {
  ...
  await cyclin.evaluate((element) => {
    globalThis.__m7CyclinRow = element;
  });
  await buyOne.click();
  await expect
    .poll(() => cyclin.evaluate((element) => element === globalThis.__m7CyclinRow))
    .toBe(true);
...
test("M7 production dist is playable, persistent, accessible, and free of browser errors", async ({
  page,
}, testInfo) => {
  ...
  await page.screenshot({ path: testInfo.outputPath("m7_minimal_playable.png"), fullPage: true });
  expect(diagnostics).toEqual([]);
});

test("M7 production dist exposes debug mutation controls only on the explicit URL flag", async ({
  page,
}) => {
  const diagnostics = installDiagnostics(page);
...
test("M7 production dist awards meaningful offline production through the debug reload control", async ({
  page,
}) => {
  const diagnostics = installDiagnostics(page);
  await page.goto("/?debug=1");
...
test("M7 production dist reports clock skew separately and visibly protects corrupt storage", async ({
  page,
}) => {
  const diagnostics = installDiagnostics(page);
  await page.goto("/");
...
test("M7 production dist remains usable at a narrow reduced-motion viewport", async ({
  browser,
}) => {
...
test("M7 production action targets meet the 44px contract without narrow overflow", async ({
  browser,
}) => {
