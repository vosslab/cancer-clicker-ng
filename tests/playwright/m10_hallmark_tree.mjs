...
import { m10BrowserFixtureSave } from "../m10_browser_fixture.mjs";
...
const BRANCHES = [
  ...
  },
  {
    ...
    saved: (state) => {
      ...
      expect(state.regions[0].vesselLinkIds).toEqual(["vessel:m10-perfusion"]);
    },
  },
  {
    ...
    saved: (state) => {
      expect(state.committedCellCommitments).toEqual({ "m10-route": 1 });
...
function currentFixture(branch, options) {
  const envelope = JSON.parse(m10BrowserFixtureSave(branch, options));
...
async function seedFixture(page, branch, options) {
  await page.addInitScript(
    ({ key, raw }) => {
      if (window.sessionStorage.getItem("m10-fixture-seeded") !== "1") {
        window.localStorage.setItem(key, raw);
        window.sessionStorage.setItem("m10-fixture-seeded", "1");
      }
    },
...
test("M10 core-six tree exposes headings, locked status, and native branch controls", async ({
  page,
}) => {
...
test("M10 acquires each core-six branch and persists its visible decision through reload", async ({
  browser,
}) => {
...
test("M10 keeps recovery-protected hallmark mutations disabled and leaves raw storage untouched", async ({
  page,
}) => {
  const diagnostics = installDiagnostics(page);
  const corruptRaw = "{m10-corrupt-save";
...
test("M10 hallmark controls remain reachable at 360px with reduced motion and touch targets", async ({
  browser,
}) => {
  const context = await browser.newContext({
