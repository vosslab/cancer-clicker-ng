...
const CORRUPT_RAW = "{m8-corrupt-save";
...
test("M8 protects rejected raw storage until an accessible explicit replacement", async ({
  page,
}) => {
  const diagnostics = installDiagnostics(page);
  await page.addInitScript(
    ({ key, raw }) => {
      if (sessionStorage.getItem("m8-corrupt-seeded") !== "1") {
        localStorage.setItem(key, raw);
        sessionStorage.setItem("m8-corrupt-seeded", "1");
      }
    },
...
test("M8 blocks a browser storage read fault until explicit fresh-start replacement", async ({
  page,
}) => {
  const diagnostics = installDiagnostics(page);
  await page.addInitScript((key) => {
    ...
    globalThis.__m8ReadFaultActive = true;
    globalThis.__m8WriteCount = 0;
    Storage.prototype.getItem = function getItemWithM8ReadFault(candidateKey) {
      if (candidateKey === key && globalThis.__m8ReadFaultActive) throw new Error("read denied");
      return originalGetItem.call(this, candidateKey);
    };
    Storage.prototype.setItem = function countM8Writes(candidateKey, value) {
      if (candidateKey === key) globalThis.__m8WriteCount += 1;
  ...
  await expect(replace).toBeVisible();
  await divide.click({ force: true });
  expect(await page.evaluate(() => globalThis.__m8WriteCount)).toBe(0);

  await page.evaluate(() => {
    globalThis.__m8ReadFaultActive = false;
  });
  await replace.click();
  await expect(page.locator("#recovery-notice")).toHaveCount(0);
  await expect(divide).toBeEnabled();
  expect(await page.evaluate(() => globalThis.__m8WriteCount)).toBe(1);
  await divide.click();
  expect(await page.evaluate(() => globalThis.__m8WriteCount)).toBe(2);
  expect(diagnostics).toEqual([]);
});
