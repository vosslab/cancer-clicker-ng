...
import { m11BrowserFixtureSave } from "../m11_browser_fixture.mjs";
...
async function seedFixture(page) {
  const envelope = JSON.parse(m11BrowserFixtureSave());
  envelope.savedAtMs = Date.now();
  const raw = JSON.stringify(envelope);
  await page.addInitScript(
    ({ key, value }) => {
      if (window.sessionStorage.getItem("m11-fixture-seeded") !== "1") {
        window.localStorage.setItem(key, value);
        window.sessionStorage.setItem("m11-fixture-seeded", "1");
      }
    },
...
async function acquireAllM11(page) {
  for (const heading of M11_HEADINGS) {
...
test("M11 real Solid controls expose ATP conversion and all four durable branches", async ({
  page,
}) => {
  const diagnostics = installDiagnostics(page);
  await seedFixture(page);
  await page.goto("/?debug=1");
  await acquireAllM11(page);

  const metabolic = branchRow(page, M11_HEADINGS[0]);
  ...
  const immune = branchRow(page, M11_HEADINGS[1]);
  ...
  const inflammation = branchRow(page, M11_HEADINGS[2]);
  ...
  const mutation = branchRow(page, M11_HEADINGS[3]);
  ...
  await metabolic.getByRole("button", { name: "Convert substrate to ATP" }).click();
  await page.getByRole("button", { name: "Fast-forward 60 seconds" }).click();
  const cards = mutation.locator(".m11-mutation-card");
...
test("M11 recovery protection disables live hallmarks and preserves unreadable raw storage", async ({
  page,
}) => {
  const diagnostics = installDiagnostics(page);
  const corruptRaw = "{m11-corrupt-save";
...
test("M11 controls remain keyboard reachable and touch-sized at 360px in reduced motion", async ({
  browser,
}) => {
