...
test("M10 manual divisions consume viable reserve to, but never below, the banked floor", () => {
...
test("M10 elapsed reserve and perfusion maintenance are segmentation invariant at whole-second crossings", () => {
  const initial = elapsedFixture();
...
test("M10 unpaid perfusion deterministically removes its capacity and oxygen benefit", () => {
...
test("M10 unprotected exhaustion halts producer output until a refill restores capacity", () => {
...
test("M10 advanceLiveTick and full offline replay agree across exhaustion and unpaid upkeep", () => {
...
test("M10 advanceLiveTick and offline replay retain a refilled positive floor", () => {
...
test("M10 live and offline ticks share elapsed reserve and ATP outcomes", () => {
  const state = elapsedFixture();
