...
| M21 | Complete 2026-08-28       | M19, M20 complete            | balance-laboratory lane             | Visible-decision-surface simulator, five versioned scenarios, and generated calibration reports                         | Accepted: five named player models run across five scenarios through the canonical visible decision surface, with aggregate evidence in `docs/BALANCE.md`.                               |
| M22 | Local complete 2026-08-28 | M21 acceptance; M18 complete | release-evidence/documentation lane | Durable documentation refresh, visual-first board convergence, current screenshots, release evidence, and Pages handoff | Local gates, the seven-frame capture, and independent image review are accepted. Human Git review, remote Pages execution, and live-site acceptance remain the release handoff.          |
...
The current queue first refreshes local release evidence: run final static, build, browser, and
Python gates; capture and independently review all seven frames; then insert the results in
`docs/RELEASE_EVIDENCE.md`. The human release owner subsequently reviews the candidate, stages the
intended files, and verifies the deployed Pages artifact.
...
| M22 visual-first convergence | Board/HUD/tumor/evolution/rack/icon/tooltip source, permanent semantic tests, seven-frame capture, independent image review, fresh final gates, release evidence, and human handoff manifest. |
...
Git review, staging, commits, pushes, remote workflow execution, and live-site acceptance are
human-owned external handoff steps. M22 provides the ready-to-stage manifest, installed local
workflow candidate, validated commands, and artifact inventory for that handoff without
representing human release actions as complete.
