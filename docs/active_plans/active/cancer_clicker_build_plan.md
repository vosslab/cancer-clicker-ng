...
| M4  | Complete 2026-08-27 | M2 and M3 complete                 | state/persistence lane     | `src/state/*`, save fixtures, migration tests                          | Exact save round-trip, migration, visible safe recovery, exhaustive event funnel                    | Accepted; final terminal reports                                                                                                         |
| M5  | Complete 2026-08-27 | M4 accepted                        | offline/state lane         | `docs/GAME_DESIGN.md`, `src/state/offline.ts`, report, tests           | Offline and live simulation agree within 2 percent                                                  | Accepted: systems `2f8a`, boundary `f4a1`, oracle `20260827`; 32 focused, 52 Node                                                        |
...
| M10 | Complete 2026-08-27 | M2 and M9 accepted                 | core-hallmarks/UI lane     | Six core hallmarks and `hallmark_tree.ts`                              | Each branch measurably changes purchase ordering                                                    | Accepted: semantic, boundary/security, offline; `check_codebase` 5/5, 164 Node/tsx, Pages build, 15/15 browser, diff                    |
| M11 | In progress 2026-08-27 | M10 accepted; amendment announced | 2011-hallmarks/UI lane  | Four 2011 hallmarks and closed event amendment                          | ATP has a sink; all four change purchase ordering                                                   | Amendment owner: `/root/m11_contract_amendment`; `m11_contract_amendment.27aug.report.md`; source lanes await typed catalog contract |
...
| M13 | Complete 2026-08-27 | M2 accepted                        | prestige-design lane       | `docs/PRESTIGE_DESIGN.md`, `docs/SYSTEM_INTERACTIONS.md`               | Four reset identities and 10-20 interactions specified                                              | Accepted: design, systems/balance, implementation contract; 18 interactions                                                              |
...
| M17 | Complete 2026-08-27 | M16 accepted                       | colony-layout/SVG lane     | `src/svg/colony_layout.ts`, `src/svg/colony_metrics.ts`                | Data-only geometry preserves stage-family separation with suppressed detail                         | Accepted: final repair, geometry, types/architecture, and macro separation; 9/9 focused, 195 Node/tsx, zero forbidden geometry violations |
| M18 | Not started         | M17 accepted                       | cell-rendering/SVG lane    | SVG cells, defs, descriptions, icons, visual tests                     | Volume, uniqueness, family separation, and node budget proven                                       | Dispatchable: consume M17 readonly slots and physical suppressed-detail geometry; own all SVG, DOM, browser, and visual evidence        |
...
1. M11 owner `/root/m11_contract_amendment`: announce the exact three-event amendment before
   source work, then hand off the four 2011 hallmark branches. Success: ATP has a real sink and
   each branch changes an observed purchase order through its durable mechanic, with canonical
   Node/tsx and production-dist browser proof.
2. M18 owner: render M17's accepted readonly layout without changing its geometry contract. Success:
   one accessible inline SVG proves the physical suppressed-detail handoff, visual distinction,
   target-size readability, DOM budget, and production-browser behavior.
