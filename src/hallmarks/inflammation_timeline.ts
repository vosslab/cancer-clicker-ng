...
/** extended-hallmark keeps episodes short enough to be a tactical decision rather than background state. */
...
export type InflammationRegionalModifier = Readonly<{
  regionId: RegionId;
...
export type InflammationModifiers = Readonly<{
  byRegion: Readonly<Record<string, InflammationRegionalModifier>>;
  routeDiscoveryOpportunity: number;
  damagePressure: number;
  immunePressure: number;
}>;
export type InflammationTimelineProjection = Readonly<{
  episodes: readonly InflammationEpisode[];
  regionalInflammation: Readonly<Record<string, number>>;
  modifiers: InflammationModifiers;
}>;
...
function timelineClock(state: GameState, elapsedMs: number): number {
  ...
  const baseClock = extendedHallmarkElapsedClock(state);
...
export function projectInflammationTimeline(
  state: GameState,
  elapsedMs: number,
): InflammationTimelineProjection {
...
export function applyInflammationTimeline(state: GameState, elapsedMs: number): GameState {
  const projection = projectInflammationTimeline(state, elapsedMs);
  return {
    ...state,
