...
import type { AtpSinkId } from "./m11_types.js";
...
function ownsMetabolism(state: GameState): boolean {
  const definition = m11HallmarkDefinition("metabolic_deregulation");
...
export function hasFundedAtpAcceleration(state: GameState): boolean {
  ...
  if (
    budget === 0 ||
    !hasReachedM11Unlock(state.currentStage, "metabolic_deregulation") ||
 ...
 * Call this after M10 perfusion maintenance so a scarce balance cannot fund both in reverse order.
 */
