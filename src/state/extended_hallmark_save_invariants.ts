...
import { assertGeneratedMutationDraftOffer } from "../hallmarks/mutation_offer_generator.js";
import { MAX_CONCEALMENT_TOKENS } from "../hallmarks/handlers/immune_visibility.js";
import type { GameState } from "../types/state.js";

const EXTENDED_HALLMARK_SAVE_FIELDS = new Set([
  "atp",
  "atpBudget",
  "atpSinks",
...
function assertVisibilityAndEpisodes(state: GameState): void {
  ...
  const extendedHallmarkElapsedClock = state.activeTimeMs + state.totalOfflineMs;
  if (!natural(extendedHallmarkElapsedClock))
  ...
  for (const episode of state.inflammationEpisodes) {
    if (
      ...
      episode.deadlineMs <= extendedHallmarkElapsedClock
    ) {
...
function assertMutationOffers(state: GameState): void {
  ...
  const canonicalMutationIds = MUTATION_DRAFT_CARD_CATALOG.map((card) => card.id);
  ...
  if (!MUTATION_DRAFT_OFFER_THRESHOLDS.some((threshold) => threshold.burden === offer.threshold)) {
...
/** ASVS 2.2.3, 2.3.3, 15.3.5, and 15.3.6: validate all extended-hallmark durable relations together. */
export function assertExtendedHallmarkSaveInvariants(state: GameState): void {
  assertAtpBudget(state);
  assertVisibilityAndEpisodes(state);
  assertMutationOffers(state);
}

/** extended-hallmark records are semantic state; a bad leaf cannot be silently recovered into a new save. */
export function hasExtendedHallmarkRecoveryNotice(field: string): boolean {
  return EXTENDED_HALLMARK_SAVE_FIELDS.has(field);
}
